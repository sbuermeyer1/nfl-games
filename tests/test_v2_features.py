...
def test_manifested_values_are_numeric_finite_and_missing_sides_are_explicitly_imputed():
    ...
    assert game["neutral_pass_rate_diff"] == pytest.approx(-0.40)
    all_columns = tuple(
        dict.fromkeys(
            column
            for target in ("margin", "total")
...
def test_manifest_sha256_is_deterministic_over_sorted_inputs():
    first = _build(
        sources={"style": "pbp-v2", "ratings": "pbp-v1"},
        constants={"z": 2, "a": 1},
    ).manifest
    reordered = _build(
        sources={"ratings": "pbp-v1", "style": "pbp-v2"},
        constants={"a": 1, "z": 2},
    ).manifest
    changed = _build(
        sources={"ratings": "pbp-v3", "style": "pbp-v2"},
        constants={"a": 1, "z": 2},
    ).manifest
...
def test_frozen_neutral_fill_constant_cannot_be_overridden():
    with pytest.raises(ValueError, match="neutral fill"):
        _build(constants={"block_neutral_fill": 1.0})
