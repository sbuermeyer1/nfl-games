...
class _TargetPredictor(Protocol):
    def predict(self, matrix: pd.DataFrame) -> object: ...
...
def select_target_config(
    ...
    *,
    ...
    ``evaluations`` has one row per configuration and inner validation season, with
    ``config``, ``validation_season``, ``mae``, and ``n_games`` columns. Seasons have
    equal weight regardless of the number of games they contain.
    """
    ...
    best_mae = min(selection.mean_inner_mae for selection in eligible)
    candidate_best: dict[str, float] = {}
    for selection in eligible:
        candidate_best[selection.config.candidate] = min(
            candidate_best.get(selection.config.candidate, float("inf")),
            selection.mean_inner_mae,
        )
    simple_candidates = [
        candidate
    ...
    return min(
        within_candidate,
        key=lambda selection: (selection.mean_inner_mae, selection.config.key()),
    )
...
def _default_target_fitter(
    ...
    *,
    ...
    return fit_target_ridge(train, target, config, manifest)  # type: ignore[arg-type]
...
def _predict_target(
    ...
    *,
    ...
    columns = list(manifest.columns(_target_manifest_name(target), config.candidate))
    predicted = np.asarray(fitted.predict(validation.loc[:, columns]), dtype=float)
...
def _inner_evaluations(
    ...
    return pd.DataFrame(
        rows,
        columns=["config", "validation_season", "mae", "n_games"],
    )
